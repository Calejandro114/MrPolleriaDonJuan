...
/* js/componentes/ui-nav.js */
import { updateCartBadgeUI } from './cart-service.js';
...
export function renderNavUI() {
    ...
    // Inicializar el conteo visual actual del carrito
    updateCartBadgeUI();

    // 1. Botón Inicio: Scroll suave en index.html
    const inicioBtn = document.getElementById("nav-inicio-btn");
    if (inicioBtn && !esSubcarpeta) {
        inicioBtn.addEventListener("click", (e) => {
            e.preventDefault();
    ...
    // 2. Botón FAQ: Scroll suave al footer
    const faqBtn = document.getElementById("nav-faq-btn");
    if (faqBtn) {
        faqBtn.addEventListener("click", (e) => {
            const footerContainer = document.getElementById("footer-container");
    ...
    // 3. Manejador Global de Scroll para Navbar
    const navbar = document.querySelector(".sticky-navbar");
    if (navbar) {
        const handleScroll = () => {
            const scrollY = window.scrollY;
